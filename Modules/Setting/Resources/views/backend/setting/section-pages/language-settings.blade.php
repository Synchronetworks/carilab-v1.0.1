...
<div class="container">
    <div class="col-md-12 d-flex justify-content-between mb-4">
        <h4><i class="ph ph-translate"></i> {{ __('messages.lbl_language') }}</h4>
    ...
    <div class="row gy-4">
        <div class="col-md-6">
            <div class="form-group">
                <label for="language_option" class="col-sm-12 form-label">{{ __('messages.lbl_language') }}</label>
                <div class="col-sm-12">
                    <select class="form-select select2 select2js opt-LANGUAGE" name="language_option[]" id='change_lang'>
                        @if(count($languages) > 0)
                            @foreach( $languages  as $lang )
                                <option value="{{$lang['id']}}" {{ config('app.locale') == $lang['id']  ? 'selected' : '' }} >{{$lang['name']}}</option>
                            @endforeach
                        @endif
                    </select>
                </div>
        ...
        <div class="col-md-6">
            <div class="form-group">
                <label for="selected_file" class="col-sm-12 form-label">{{ __('messages.select_file') }}</label>
                <div class="col-sm-12">
                    <select class="form-select select2 select2js opt-LANGUAGE" name="selected_file[]" id="selected_file">
...
<script>
    function getLangFile(lang='', file=''){
        var url = "{{ route('backend.settings.getLangFile') }}";
        $.ajax({
            type: 'post',
            url: url,
            headers: {
