...
<form method="POST" action="{{ route('backend.setting.store') }}">
    @csrf
    <div>
        <h3><i class="fa-solid fa-sliders"></i> {{ __('messages.lbl_integration') }} </h3>
    </div>
    ...
    <div id="google-key-field" class="{{ old('is_google_login', $data['is_google_login'] ?? 0) == 1 ? '' : 'd-none' }}">
        <div class="row">
            <div class="col-md-6">
                <input type="text" name="google_secretkey" id="google_secretkey" class="form-control @error('google_secretkey') is-invalid @enderror" value="{{ old('google_secretkey', $data['google_secretkey'] ?? '') }}" placeholder="{{ __('messages.lbl_secret_key') }}" {{ old('is_google_login', $data['is_google_login'] ?? 0) == 1 ? '' : 'disabled' }}>
                @error('google_secretkey')
                    <span class="text-danger">{{ $message }}</span>
                @enderror
            </div>
            <div class="col-md-6">
                <input type="text" name="google_publickey" id="google_publickey" class="form-control @error('google_publickey') is-invalid @enderror" value="{{ old('google_publickey', $data['google_publickey'] ?? '') }}" placeholder="{{ __('messages.lbl_public_key') }}" {{ old('is_google_login', $data['is_google_login'] ?? 0) == 1 ? '' : 'disabled' }}>
    ...
    <div class="form-group border-bottom pb-3 mb-3">
        <div class="d-flex justify-content-between align-items-center">
            <label class="form-label m-0" for="is_one_signal_notification">{{ __('messages.one_signal_notification') }}</label>
    ...
    <div class="form-group border-bottom pb-3 mb-3">
        <div class="d-flex justify-content-between align-items-center">
            <label class="form-label m-0" for="is_custom_webhook_notification">{{ __('messages.custom_webhook_notification') }}</label>
    ...
    <div class="form-group border-bottom pb-3 mb-3">
        <div class="d-flex justify-content-between align-items-center">
            <label class="form-label m-0" for="is_map_key">{{ __('messages.google_map_key') }}</label>
...
<div class="form-group border-bottom pb-3 mb-3">
    <div class="d-flex justify-content-between align-items-center">
        <label class="form-label m-0" for="is_application_link">{{ __('messages.application_links') }}</label>
...
<div class="form-group border-bottom pb-3 mb-3">
    <div class="d-flex justify-content-between align-items-center">
        <label class="form-label m-0" for="isForceUpdate">{{ __('messages.force_update_customer_app') }}</label>
