    ...
    <div class="table-content">
        <x-backend.section-header>
            <div class="d-flex flex-wrap gap-3">
                @if(auth()->user()->can('edit_prescription') || auth()->user()->can('delete_prescription'))
                <x-backend.quick-action url="{{ route('backend.' . $module_name . '.bulk_action') }}">
                    <div class="">
                        <select name="action_type" class="form-select select2 col-12" id="quick-action-type"
                            ...
                            @can('delete_prescription')
                            <option value="delete">{{ __('messages.delete') }}</option>
                            @endcan
                            @can('restore_prescription')
                            <option value="restore">{{ __('messages.restore') }}</option>
                            @endcan
                            @can('force_delete_prescription')
...
<script type="text/javascript" src="{{ asset('vendor/datatable/datatables.min.js') }}"></script>
    <script type="text/javascript" defer>
        const columns = [
            {
                name: 'check',
                data: 'check',
                title: '<input type="checkbox" class="form-check-input" name="select_all_table" id="select-all-table" data-type="prescription" onclick="selectAllTable(this)">',
                width: '0%',
                exportable: false,
                orderable: false,
                searchable: false,
                visible: {!! auth()->user()->can('edit_prescription') || auth()->user()->can('delete_prescription') ? 'true' : 'false' !!}
            ...
            },
            {
                data: 'uploaded_at',
                name: 'uploaded_at',
                title: "{{ __('messages.lbl_uploaded_at') }}"
            },
            {
                data: 'lab',
                name: 'lab',
        ...
        document.addEventListener('DOMContentLoaded', (event) => {
            ...
            initDatatable({
                url: '{{ route("backend.$module_name.index_data") }}' + '?approval_status=' + approval_status,
                finalColumns,
                orderColumn: [[ 4, "desc" ]],
                advanceFilter: () => {
                    return {
