...
<div class="d-flex justify-content-end align-items-center flex-wrap gap-3 mb-3">
    <a href="{{ route('subscriptionPlan') }}" class="btn btn-sm btn-primary">
                    ...
                    <form action="{{ route('process-payment') }}" method="POST" id="payment-form">
                        ...
                        <div class="bg-body p-3 rounded mb-4">
                            <div class="form-group">
                                <input type="hidden" id="subscription-type" name="subscription_type" value="{{ $subscriptionType }}">
                                <input type="hidden" id="selected-plan-id" name="plan_id" value="{{ $planId }}">
...
<script>
   document.addEventListener("DOMContentLoaded", function() {
    ...
    var selectedPlanId = $('#selected-plan-id').val(); // Injected from backend
    ...
    $('.payment-radio-btn').on('change', function () {
        ...
        function updatePaymentDetails(planId) {
            ...
            url: `{{ route('get-payment-details') }}`,
                ...
                error: function(xhr) {
                    $('#errorModalMessage').text("{{ __('messages.an_error_occurred_while_fetching_payment_details') }}");
                    var errorModal = new bootstrap.Modal(document.getElementById('errorModal'));
                    errorModal.show();
                }
            });
...
function paymentSubmit(){
    let paymentForm=document.getElementById('payment-form')
        ...
        const subscriptionPayment = $('#subscription-type').val();
        ...
        formData += '&subscription_type=' + encodeURIComponent(subscriptionPayment);
            $.ajax({
                ...
                error: function(xhr) {
                    ...
                    const errorMessage = errorResponse.error || "{{ __('messages.an_error_occurred_payment_method') }}";
