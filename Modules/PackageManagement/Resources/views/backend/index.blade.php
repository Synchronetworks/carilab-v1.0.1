    ...
    <x-backend.advance-filter>
        ...
        <form action="javascript:void(0)" class="datatable-filter">
            <div class="row gy-4">
                ...
                <div class="form-group">
                    <label class="form-label" for="category_id">{{ __('messages.lbl_test_category') }}</label>
                    <select name="filter_category" id="category_id" data-filter="select" class="select2 form-control"
                        ...
                        data-ajax--cache="true"
                        data-placeholder="{{ __('messages.select_test_category') }}">
                        <option value="">{{ __('messages.select_test_category') }}</option>
                    </select>
                </div>

                {{-- Test Case Filter --}}
                <div class="form-group">
                    <label class="form-label" for="test_id">{{ __('messages.lbl_test_case') }}</label>
                    <select name="filter_test" id="test_id" data-filter="select" class="select2 form-control"
                        data-ajax--url="{{ route('backend.get_search_data', ['type' => 'test']) }}" 
                        data-ajax--cache="true"
                        data-placeholder="{{ __('messages.select_test_case') }}">
                        <option value="">{{ __('messages.select_test_case') }}</option>
                    </select>
                </div>
            </div>
        </form>
...
@push('after-scripts')
    ...
    <script type="text/javascript" defer>
        const columns = [{
                name: 'check',
                data: 'check',
                title: '<input type="checkbox" class="form-check-input" name="select_all_table" id="select-all-table" data-type="packagemanagement" onclick="selectAllTable(this)">',
                width: '0%',
                exportable: false,
                orderable: false,
                searchable: false,
                visible: {!! auth()->user()->can('edit_packages') || auth()->user()->can('delete_packages') ? 'true' : 'false' !!}
            },
            {
                data: 'updated_at',
                name: 'updated_at',
            ...
            },
            {
                data: 'discount_price',
                name: 'discount_price',
                title: "{{ __('messages.lbl_discount_price') }}"
            },
            {
                data: 'status',
                name: 'status',
