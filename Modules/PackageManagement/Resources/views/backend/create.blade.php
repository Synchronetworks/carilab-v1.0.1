...
@section('title', __('messages.new_package'))

@section('content')
    <div class="form-content">
        <form action="{{ route('backend.packagemanagements.store') }}" method="POST" enctype="multipart/form-data" class='requires-validation' id="form-submit" novalidate>
            @csrf
                    ...
                    <div class="row gy-4">                                   
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="file" name="package_image" id="packageimageInput" class="form-control d-none" accept="image/png,image/jpeg">
                            ...
                            <div class="row gy-4">
                                <div class="col-md-12">
                                    ...
                                    <input type="text" class="form-control" name="name" id="name" 
                                        value="{{ old('name') }}" placeholder="{{ __('messages.enter_package_name') }}" required>
                    ...
                    <div class="row gy-4">                                
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="catalog_id[]" id="catalog_id" class="form-select select2" multiple required>
                                <option value="">{{ __('messages.placeholder_select_case') }}</option>
                            </select>
            ...
            <div class="card">
                <div class="card-body">                    
                    <div class="row gy-4">
                        <div class="col-md-4">
                            <label class="form-label">{{ __('messages.price') }} ({{getCurrencySymbol()}})<span class="text-danger">*</span></label>
                            <input type="number" name="price" class="form-control" min="0" step="any" value="{{ old('price') }}" placeholder="{{ __('messages.enter_price') }}" required>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="date" class="form-control datetimepicker" name="start_at" id="start_at" 
                                value="{{ old('start_at') }}" placeholder="{{ __('messages.enter_start_date') }}" required>
                            @error('start_at') <span class="text-danger">{{ $message }}</span> @enderror
                            <div class="invalid-feedback" id="start_at-error">{{__('messages.start_date_required')}} </div>
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="date" class="form-control datetimepicker" name="end_at" id="end_at" 
                                value="{{ old('end_at') }}" placeholder="{{ __('messages.enter_end_date') }}" required>
                            @error('end_at') <span class="text-danger">{{ $message }}</span> @enderror
                            <div class="invalid-feedback" id="end_at-error">{{__('messages.end_date_required')}} </div>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="row gy-4">
                        <div class="col-md-4">
                            ...
                            <div class="d-flex align-items-center justify-content-between form-control">
                                ...
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" id="discount" name="is_discount" {{ old('is_discount') == 1 ? 'checked' : '' }}>
                                </div>
                            </div>
                        ...
                        <div class="col-md-4" id="discount_type">
                            ...
                            <select name="discount_type" id="discount_type" class="form-select select2">
                                <option value="" selected>{{ __('messages.select_discount_type') }}</option>
                        ...
                        <div class="col-md-4" id="discount_price">
                            <label for="discount_price" class="form-label">{{ __('messages.lbl_discount_price') }}</label>
                            <input type="number" class="form-control" min="0" step="any" name="discount_price" id="discount_price" 
                                value="{{ old('discount_price') }}" placeholder="{{ __('messages.enter_discount_price') }}">
            ...
            <!-- Status -->
            <div class="card-input-title mb-3">
                <h4 class="m-0">{{ __('messages.status') }}</h4>
            </div>
            <div class="card">
                <div class="card-body">
                     <div class="row gy-4">
                            ...
                            <div class="d-flex align-items-center justify-content-between form-control">
                                ...
                                <div class="form-check form-switch">
                                    ...
                                    <input class="form-check-input" type="checkbox" id="is_featured" name="is_featured" 
                                        value="1" {{ old('is_featured', 0) == 1 ? 'checked' : '' }}>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            ...
                            <div class="d-flex align-items-center justify-content-between form-control">
                                ...
                                <div class="form-check form-switch">
                                    ...
                                    <input class="form-check-input" type="checkbox" id="is_home_collection_available" name="is_home_collection_available" 
                                        value="1" {{ old('is_home_collection_available', 0) == 1 ? 'checked' : '' }}>
                                </div>
                            </div>
                        </div>
                    </div> 
...
<script>
    ...
    $(document).ready(function() {
        $('#catalog_id').select2({
                    placeholder: "{{ __('messages.placeholder_select_case') }}",
                    allowClear: true
                });  
...
$removeButton.on('click', function() {
    ...
    $(this).hide();
        ...
        let oldDiscount = @json(old('is_discount', false)); // Default to `false` if not set
