...
<div class="table-content">
    <x-backend.section-header>
        <div class="d-flex flex-wrap gap-3">
            @if(auth()->user()->can('edit_collectordocuments') || auth()->user()->can('delete_collectordocuments'))
            <x-backend.quick-action url="{{ route('backend.collectordocument.bulk_action') }}">
                <div class="">
                    <select name="action_type" class="form-select select2 col-12" id="quick-action-type"
                        ...
                        @endcan 
                        @can('delete_collectordocuments')
                        <option value="delete">{{ __('messages.delete') }}</option>
                        @endcan
                        @can('restore_collectordocuments')
                        <option value="restore">{{ __('messages.restore') }}</option>
                        @endcan
                        @can('force_delete_collectordocuments')
        ...
        <x-slot name="toolbar">
            ...
            <div class="input-group flex-nowrap">
              ...
              @hasPermission('add_collectordocuments')
              <a href="{{ route('backend.collectordocument.create',['collectordocument' => $collectordata->id??' ']) }}" class=" btn btn-primary d-flex align-items-center gap-1"><i class="ph ph-plus-circle align-middle"></i>{{ __('messages.new') }}</a>
              @endhasPermission
  ...
  <script type="text/javascript" defer>
    ...
    const  columns= [{
            ...
            orderable: false,
            searchable: false,
            visible: {!! auth()->user()->can('edit_collectordocuments') || auth()->user()->can('delete_collectordocuments') ? 'true' : 'false' !!}
      ...
      document.addEventListener('DOMContentLoaded', (event) => {
            initDatatable({
                url: '{{ route("backend.collectordocument.index_data", ["collectordocument" => $collectordata->id??'null']) }}',
                finalColumns,
                orderColumn: [  [1, 'desc']]
