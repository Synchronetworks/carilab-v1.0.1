...
<div class="table-content">
    <x-backend.section-header>
        <div class="d-flex flex-wrap gap-3">
            @if(auth()->user()->can('edit_plans') || auth()->user()->can('delete_plans'))
        ...
        <x-slot name="toolbar">
            ...
            <a href="{{ route('backend.' . $module_name . '.create') }}" class="btn btn-primary d-flex align-items-center gap-1"
                id="add-post-button"> <i class="ph ph-plus-circle align-middle"></i>{{__('messages.new')}}</a>
                @endhasPermission
...
<x-backend.advance-filter>
    ...
    <form action="javascript:void(0)" class="datatable-filter">
        <div class="form-group">
            ...
            <label class="form-label" for="name">{{ __('messages.lbl_name') }}</label>
            <input type="text" name="name" id="name" class="form-control"
                placeholder="">
        </div>

        <div class="form-group">
        <label for="level" class="form-label">{{ __('messages.lbl_level') }}</label>
        ...
        <div class="form-group">
                <label for="level" class="form-label">{{ __('messages.lbl_amount') }}</label>

                <select class="form-select select2" name="price" id="price" required>
                <option value="">{{ __('messages.select_price') }}</option>
...
const columns = [
            {
                ...
                orderable: false,
                searchable: false,
                visible: {!! auth()->user()->can('edit_plans') || auth()->user()->can('delete_plans') ? 'true' : 'false' !!}
            ...
            { 
                ...
                title: "{{ __('messages.lbl_discount') }}" 
            },
            { 
                data: 'total_price',
                name: 'total_price', 
