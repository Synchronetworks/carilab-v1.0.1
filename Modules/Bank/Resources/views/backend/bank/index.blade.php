        ...
        <x-backend.section-header>
            ...
            <x-slot name="toolbar">
                ...
                <div class="input-group flex-nowrap">
                    ...
                    @hasPermission('add_collector_bank')
                    <a href="{{ route('backend.' . $user_type . '_bank.create') }}" class="btn btn-primary" id="add-post-button"> {{ __('messages.add_bank') }}</a>
                @endhasPermission
            </x-slot>
        </x-backend.section-header>
...
<script type="text/javascript" src="{{ asset('vendor/datatable/datatables.min.js') }}"></script>
    <script type="text/javascript" defer>
        const columns = [
            {
                name: 'check',
                data: 'check',
                title: '<input type="checkbox" class="form-check-input" name="select_all_table" id="select-all-table" data-type="bank" onclick="selectAllTable(this)">',
                width: '0%',
                exportable: false,
                orderable: false,
                searchable: false,
                visible: {!! auth()->user()->can($permissions['edit']) || auth()->user()->can($permissions['delete']) ? 'true' : 'false' !!}
            ...
            },
            {
                data: 'branch_name',
                name: 'branch_name',
                title: "{{ __('messages.lbl_branch_name') }}"
            },
            {
                data: 'user_name',
                name: 'user_name',
                title: "{{ request()->route('user_type') === 'collector' ? __('messages.lbl_collector_name') : __('messages.lbl_vendorName') }}"
        ...
        document.addEventListener('DOMContentLoaded', (event) => {
            const userType = '{{ request()->route('user_type') }}';
            initDatatable({
