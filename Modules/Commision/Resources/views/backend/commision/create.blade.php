@extends('backend.layouts.app')
@section('title'){{__($module_title)}}@endsection


@section('content')
<div class="form-content">


    {{ html()->form('POST' ,route('backend.commisions.store'))->attribute('enctype', 'multipart/form-data')->attribute('data-toggle', 'validator')           
        ->attribute('id', 'form-submit')  // Add the id attribute here
        ->class('requires-validation')  // Add the requires-validation class
        ->attribute('novalidate', 'novalidate')  // Disable default browser validation
        ->open() }}
        @csrf
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-3">
            <div class="card-input-title">
                <h4 class="m-0">{{ __('messages.basic_information') }}</h4>
            </div>
            <a href="{{ route('backend.'. $user_type .'_commisions.index') }}" class="btn btn-sm btn-primary">
                <i class="ph ph-caret-double-left"></i> {{ __('messages.back') }}
            </a>
        </div>  
        {{ html()->hidden('user_type', $user_type) }}
        <div class="card tfhtghrugi">
            <div class="card-body">
                <div class="row gy-3">
                    <div class="col-md-6">
                        {{ html()->label(__('messages.lbl_name') . ' <span class="text-danger">*</span>', 'title')->class('form-label') }}
                        {{ html()->text('title')
                                ->attribute('value', old('title'))
                                ->placeholder(__('messages.placeholder_name'))
                                ->class('form-control')
                                ->required() }}
                        @error('title')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                        <div class="invalid-feedback" id="name-error">{{__('messages.name_required')}}</div>
                    </div>
                    <div class="col-md-6">
                        {{ html()->label(__('messages.lbl_Type') . ' <span class="text-danger">*</span>', 'type')->class('form-label') }}
                        {{ html()->select('type', ['' => __('messages.select_type'), 'Fixed' => __('messages.fixed'), 'Percentage' => __('messages.percentage')])
                                ->class('form-select select2')
                                ->required()
                                ->attributes(['oninvalid' => "this.setCustomValidity('" . __('messages.type_required') . "')", 'oninput' => "this.setCustomValidity('')"]) }}
                        @error('type')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                        <div class="invalid-feedback" id="name-error">{{ __('messages.type_required') }}</div>
                    </div>
                    <div class="col-md-6">
                        {{ html()->label(__('messages.lbl_value') . ' <span class="text-danger">*</span>', 'value')->class('form-label') }}
                        {{  html()->number('value')
                      
                                ->attribute('min', '0')
                                ->attribute('step', 'any')
                                ->attribute('oninput', "validity.valid||(value='')")
                                ->attribute('value', old('value'))
                                ->placeholder(__('messages.placeholder_value'))
                                ->class('form-control')
                                ->required() }}
                        @error('value')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                        <div class="invalid-feedback" id="name-error">{{__('messages.value_required')}}</div>
                    </div>
                    <div class="col-md-6">
                        {{ html()->label(__('messages.lbl_status'), 'status')->class('form-label') }}
                        <div class="d-flex align-items-center justify-content-between form-control">
                            {{ html()->label(__('messages.active'), 'status')->class('form-label mb-0 text-body') }}
                            <div class="form-check form-switch">
                                {{ html()->hidden('status', 1) }}
                                {{
                                html()->checkbox('status',old('status', true))
                                    ->class('form-check-input')
                                    ->id('status')
                                }}
                            </div>
                            @error('status')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>

                    </div>
                </div>
            </div>
        </div>
        <div class="d-flex justify-content-end gap-3">
            {{ html()->submit(trans('messages.save'))->class('btn btn-md btn-primary')->id('submit-button') }}
        </div>
    {{ html()->form()->close() }}
</div>
@endsection
