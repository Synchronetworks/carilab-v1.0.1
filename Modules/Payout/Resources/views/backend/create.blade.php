...
@section('content')
    <div class="form-content">
        ...
        <form action="{{ route('backend.payouts.store') }}" method="POST" class="requires-validation" id="form-submit" novalidate>
            @csrf
            <div class="card">
                <div class="card-body">                    
                    {{ html()->hidden('user_id',$collector->id ?? null) }}
                    {{ html()->hidden('user_type',$collector->user_type ?? null) }}
                    <!-- Basic Information -->
                    <div class="row gy-4">
                        <div class="col-md-4">
                            ...
                            <select name="payment_method" id="method" class="form-control select2" required>
                                @if($collector->banks->count() > 0)
                        ...
                        <div class="col-md-4 d-none" id="select_bank">
                            ...
                            <select name="bank" id="bank" class="form-control select2">
                                ...
                                @foreach($collector->banks as $bank)
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <input type="text" class="form-control" value="{{ Currency::format($collectorearning ?? 0) }}" readonly>
                            <input type="hidden" name="amount" value="{{ old('amount', $collectorearning) }}">
                        </div>
                        ...
                        <div class="form-group col-md-12 d-none" id="payment_gateway">
                            ...
                            <div class="d-flex align-items-center flex-sm-nowrap flex-wrap gap-3">
                                ...
                                <label class="form-check form-control px-5 cursor-pointer">
                                    ...
                                    <span class="form-check-label">{{__('messages.lbl_stripe')}} </span>
                                </label>
                            </div>
