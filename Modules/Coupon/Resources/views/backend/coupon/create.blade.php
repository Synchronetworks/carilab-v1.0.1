...
<div class="form-content">
    ...
    {{ html()->form('POST', route('backend.coupons.store'))
            ...
            <div class="card-body">           
                <div class="row gy-4">
                    ...
                    <div class="col-sm-6 col-md-4">
                        {{ html()->label(__('messages.select_vendor'), 'vendor_id')->class('form-label') }}
                        {{ html()->select('vendor_id', $vendors->pluck('full_name', 'id')->prepend(__('messages.select_vendor_optional'), '')->toArray())
                            ->class('form-control') }}
                        @error('vendor_id')
                    ...
                    <div class="col-sm-6 col-md-4">
                        ...
                        {{ html()->text('coupon_code')->placeholder(__('messages.enter_coupon_code'))
                            ->class('form-control') }}
                        @error('coupon_code')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="col-sm-6 col-md-4">
                        ...
                        {{ html()->select('discount_type', ['percentage' => __('messages.percentage'), 'fixed' => __('messages.fixed')])
                            ->placeholder(__('messages.select_discount_type'))
                            ->class('form-control') }}
                        @error('discount_type')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
...
<select name="applicability[]" id="applicability" class="form-select select2" multiple required placeholder="{{ __('messages.applicability') }}">
    <option value="" disabled>{{ __('messages.select_name', ['select' => __('messages.applicability')]) }}</option>
    <option value="specific_tests">{{ __('messages.specific_tests') }}</option>
    <option value="specific_packages">{{ __('messages.specific_packages') }}</option>
    <option value="all">{{ __('messages.all_tests_packages') }}</option>
</select>
                        @error('applicability')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    ...
                    <!-- Select Test (conditionally shown) -->
                    ...
                    <div class="col-sm-6 col-md-4" id="package_select" style="display: none;">
                        <label class="form-label">{{ __('messages.packages') }}<span class="text-danger">*</span></label>
                        <div class="form-check">
                        ...
                        <select class=" form-control  select2" name="package_id[]" id="package_id" multiple >
                        </select>
                        @error('package_id') <span class="text-danger">{{ $message }}</span> @enderror
                        <div class="invalid-feedback">{{__('messages.package_required')}}</div>
                    </div>
                    ...
                    <div class="col-sm-6 col-md-4">
                        ...
                        {{ html()->date('start_at')
                        ->attribute('data-min-date', 'today')
                        ->class('form-control datetimepicker')
                        ->placeholder(__('messages.placeholde_select_start_date')) 
                         }}
                        @error('start_at')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="col-sm-6 col-md-4">
                        ...
                        {{ html()->date('end_at')->class('form-control datetimepicker')->attribute('data-min-date', 'today') ->placeholder(__('messages.placeholde_select_end_date'))  }}
                        @error('end_at')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="col-sm-6 col-md-4">
                        ...
                        {{ html()->number('total_usage_limit')->placeholder(__('messages.enter_total_usage'))
                            ->class('form-control') 
                            ->attribute('min', '1')}}
                    ...
                    <div class="col-sm-6 col-md-4">
                        ...
                        {{ html()->number('per_customer_usage_limit')->placeholder(__('messages.enter_per_customer_usage'))
                            ->class('form-control') 
                            ->attribute('min', '1')}}
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updateLabSelect(data) {
            const labSelect = $('#lab_id');
            labSelect.empty();
            labSelect.append('<option value="" disabled selected>{{ __("messages.select_lab") }}</option>');
            $.each(data, function(key, value) {
                labSelect.append('<option value="' + value.id + '">' + value.name + '</option>');
            }); 
        }

        // Load tests and packages when lab changes
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.data && response.data.length > 0) {
                            ...
                            $('#test_id').empty().append('<option value="">{{ __("messages.no_test_availabile") }}</option>').hide();
                        }
                    },
                    error: function(xhr) {
