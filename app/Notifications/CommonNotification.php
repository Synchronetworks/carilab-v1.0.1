...
class CommonNotification extends Notification implements ShouldQueue
{
    ...
    public function __construct($type, $data)
    {
        ...
        ->with(['defaultNotificationTemplateMap' => function($query) use ($userType) {
            $query->where('user_type', $userType);
        }])
        ->first();
        
        
        $this->subject = $this->subject = str_replace(
            array_map(fn($k) => '[[ ' . $k . ' ]]', array_keys($this->data)),
            array_values($this->data),
            $this->notification->defaultNotificationTemplateMap->subject
        );  
              
        $this->type = ucwords(str_replace('_', ' ', $this->notification->type)) . '!';
        $this->notification_message = $this->notification->defaultNotificationTemplateMap->template_detail;
            ...
            foreach ($notificationSettings as $key => $notification) {
                if ($notification) {
                    switch ($key) {
                        ...
                        case 'IS_SMS':
                            if (auth()->user()->hasRole('vendor') && !auth()->user()->enableSMSNotification()) {
                                break; // Skip sending WhatsApp notifications
                            }
                            $templateDetail = $this->notification->defaultNotificationTemplateMap->sms_template_detail ?? null;
                            ...
                            $notificationData['type'] = $this->notification->defaultNotificationTemplateMap->sms_subject ?? 'None';
                            $notificationData['message'] = $templateDetail ?? __('messages.default_notification_body');
                        
                            $this->sendSmsMessage($notificationData);
                        break;
                    case 'IS_WHATSAPP':
                        if (auth()->user()->hasRole('vendor') && !auth()->user()->enableWhatsappNotification()) {
                            break; // Skip sending WhatsApp notifications
                        }
                        $templateDetail = $this->notification->defaultNotificationTemplateMap->whatsapp_template_detail ?? null;
                        ...
                        $notificationData = $this->data;
                        $notificationData['type'] = $this->notification->defaultNotificationTemplateMap->whatsapp_subject ?? 'None';
                        $notificationData['message'] = $templateDetail ?? __('messages.default_notification_body');
    ...
    function sendSmsMessage($data) {
        ...
        if(empty(setting('sms_notification')) || setting('sms_notification')!=1){
            return false;
        }
    
        if (empty(setting('twilio_sid_sms')) || empty(setting('twilio_auth_token_sms')) || empty(setting('twilio_phone_number_sms'))) {
            return false;
        }
        if (empty($user) || empty($data['message'])) {
            return false;
        }
        $sid = setting('twilio_sid_sms');
        $authToken = setting('twilio_auth_token_sms');
        $twilioPhoneNumber = setting('twilio_phone_number_sms');
    ...
    function sendWhatsAppMessage($data) {
        ...
        $user = User::where('id', $data['person_id'])->first();
        if (empty(setting('whatsapp_notification')) || setting('whatsapp_notification') != 1) {
            return false;
        }
        if (empty(setting('twilio_sid_whatsapp')) || empty(setting('twilio_auth_token_whatsapp')) || empty(setting('twilio_whatsapp_number'))) {
            return false;  
        }
        ...
        $sid = setting('twilio_sid_whatsapp');
        $authToken = setting('twilio_auth_token_whatsapp');
        $twilioWhatsAppNumber = 'whatsapp:' . setting('twilio_whatsapp_number');
